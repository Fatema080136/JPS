...
public final class CJPSPlus implements IRouting
{
    ...
    public final List<DoubleMatrix1D> route( final ObjectMatrix2D p_objects, final DoubleMatrix1D p_currentposition, final DoubleMatrix1D p_targetposition )
    {
        ...
        {
            //l_openlist.forEach( i -> System.out.println(i.coordinate()));
            final CJumpPoint l_currentnode = l_openlist.pollFirst();
            System.out.println( l_currentnode.coordinate() );
    ...
    /**
     * pruning the neighbors of current node
     * @param p_curnode the current node to search for successors
     * @param p_objects Snapshot of the environment
     * @return l_neighbors the possible successors of current node
     */
    private ArrayList<DoubleMatrix1D> prune( final CJumpPoint p_curnode, final ObjectMatrix2D p_objects )
    {
        final ArrayList<DoubleMatrix1D> l_neighbors = new ArrayList<>();

        final double l_row = ( p_curnode.coordinate().getQuick( 0 ) - p_curnode.m_parent.coordinate().getQuick( 0 ) ) == 0 ? 0 :
                ( p_curnode.coordinate().getQuick( 0 ) - p_curnode.m_parent.coordinate().getQuick( 0 ) )/
                    Math.abs( p_curnode.coordinate().getQuick( 0 ) - p_curnode.m_parent.coordinate().getQuick( 0 ) );

        final double l_col = ( p_curnode.coordinate().getQuick( 1 ) - p_curnode.m_parent.coordinate().getQuick( 1 ) ) == 0 ? 0 :
                ( p_curnode.coordinate().getQuick( 1 ) - p_curnode.m_parent.coordinate().getQuick( 1 ) )/
                    Math.abs( p_curnode.coordinate().getQuick( 1 ) - p_curnode.m_parent.coordinate().getQuick( 1 ) );

        if( l_row != 0 && l_col != 0 )
        {
            l_neighbors.add( new DenseDoubleMatrix1D( new double[]{l_row,l_col} ) );
            l_neighbors.add( new DenseDoubleMatrix1D( new double[]{l_row,0} ) );
            l_neighbors.add( new DenseDoubleMatrix1D( new double[]{0,l_col} ) );

            if( this.diagonal( p_curnode.coordinate().getQuick( 0 ) , p_curnode.coordinate().getQuick( 1 ), -l_row, l_col, l_row, 0, p_objects ) )
            {
                l_neighbors.add( new DenseDoubleMatrix1D( new double[]{- l_row,l_col} ) );
            }

            if ( this.diagonal( p_curnode.coordinate().getQuick( 0 ), p_curnode.coordinate().getQuick( 1 ), l_row, -l_col, 0, l_col, p_objects ) )
            {
                l_neighbors.add( new DenseDoubleMatrix1D( new double[]{l_row, - l_col} ) );
            }
        }
        else
        {
            ...
            if( this.horizontal( p_curnode.coordinate().getQuick( 0 ), p_curnode.coordinate().getQuick( 1 ), l_row, p_objects, 1 ) )
            {
                l_neighbors.add( new DenseDoubleMatrix1D( new double[]{l_row, 1} ) );
            }

            if ( this.vertical( p_curnode.coordinate().getQuick( 0 ), p_curnode.coordinate().getQuick( 1 ), l_col, p_objects, 1 ) )
            {
                l_neighbors.add( new DenseDoubleMatrix1D( new double[]{1, l_col} ) );
            }
    ...
    {
        if( p_curnode.m_parent != null )
        {
            this.prune( p_curnode, p_objects ).stream()
                    .filter( j -> !this.isNotNeighbour( p_objects, p_curnode.coordinate().getQuick( 0 ) + j.getQuick( 0 ), p_curnode.coordinate().getQuick( 1 ) + j.getQuick( 1 ),
                            p_closedlist ) && !this.isOccupied( p_objects, p_curnode.coordinate().getQuick( 0 ) + j.getQuick( 0 ), p_curnode.coordinate().getQuick( 1 ) + j.getQuick( 1 ) ) )
                    .forEach( i -> {
                        final DoubleMatrix1D l_nextjumpnode = this.jump( p_curnode.coordinate(), p_target, i.getQuick( 0 ), i.getQuick( 1 ), p_objects );
